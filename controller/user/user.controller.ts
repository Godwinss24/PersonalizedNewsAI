...
import { NewsAlert } from "../../services/email.service";
dotenv.config();
...
export class UserController {
  ...
  async getMyLatest(req: CustomRequest, res: Response) {
    try {
      ...
      const newsItems = data.flatMap(({ latest }) =>
        latest.map((item) => ({
          title: item.title,
      ...
      const templateVariables = {
        contactEmail: 'support@youraiassistant.com',
        newsItems,
      };

      // BullMQ should have been used here.
      const emailService = new NewsAlert(req.user?.email || '', 'No name', templateVariables);
